from logic import build_game, print_game, move, draw, Klondike, check_move

def get_nr_of_flipped(tableau) -> int:
    result = 0
    for card in tableau:
        if card.flipped:
            result += 1
...
def find_move(g: Klondike):
    ...
    if len(g.pile) == 0:
        ...
        print_game(g)
    for j in range(0,len(g.tableaus)): # looks through different tableaus
